...
func UpdateProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		update := bson.D{{Key: "$set", Value: bson.D{{Key: "product_name", Value: UpdateProduct.Product_Name}, {"image", UpdateProduct.Image}, {"rating", UpdateProduct.Rating}, {"noserie", UpdateProduct.Num}}}}
...
func DeleteProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//matchStage := bson.D{{Key: "$match", Value: bson.D{{Key: "_id", Value: prodt_id}}}}
		defer cancel()
		count, err := ProductCollection.CountDocuments(ctx, fil)

		if count == 0 || err != nil {
			c.IndentedJSON(404, "unmatch id - no id in database")
			return
...
func SearchProductByQueryByCategory() gin.HandlerFunc {
	return func(c *gin.Context) {
		var searchProduct []models.Product
		queryParam := c.Query("category")
		//queryParamno := c.Query("no")
		...
		serachquerydb, err := ProductCollection.Find(ctx, bson.M{"category": bson.M{"$regex": queryParam}})
